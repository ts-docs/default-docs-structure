...
const THEME_LEN = Themes.length;
const DEFAULT_THEME = 0;
const icon = document.getElementById("theme-icon")!;

function setTheme(themeIndex: number) : number {
    const theme = Themes[themeIndex];
    if (!theme) return DEFAULT_THEME;
    document.body.className = theme.name;
    localStorage.setItem("theme", themeIndex.toString());
    const nextThemeIndex = (themeIndex + 1) % THEME_LEN; 
    const nextTheme = Themes[nextThemeIndex];
    icon.innerHTML = nextTheme.icon;
    return nextThemeIndex;
}

export function initTheme() {
    let currentTheme = setTheme(localStorage.getItem("theme") ? +(localStorage.getItem("theme") as string) : DEFAULT_THEME);
    icon.onclick = () => currentTheme = setTheme(currentTheme);
}