...
function setTheme(themeName: string, saveChoice?: boolean, icon?: HTMLElement) {
    document.documentElement.dataset.theme = themeName;
    if (saveChoice) localStorage.setItem("theme", themeName);
    currentTheme = themeName;
    if (icon) {
        if (themeName === "light") icon.innerHTML = Themes.dark;
        else icon.innerHTML = Themes.light;
    }
}
...
export function initTheme() {
    ...
    window.addEventListener("load", () => {
        ...
        icon.onclick = () => setTheme(oppositeOfCurrentTheme(), true, icon);
    });
}