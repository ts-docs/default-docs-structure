...
export function FunctionHead(gen: Generator, arrow: boolean, thing: {parameters?: Array<FunctionParameter>, returnType?: Type, typeParameters?: Array<TypeParameter>}) {
    return <span class="item-name">
        ...
        <>({thing.parameters ? ( isLargeSignature(thing) ? <div class="left-item">{thing.parameters.map(p => gen.generateParameter(p)).filter(p => p).join(",<br>")}</div> : thing.parameters.map(p => gen.generateParameter(p)).join(", ")) : ""})</>
        {thing.returnType ? <>
